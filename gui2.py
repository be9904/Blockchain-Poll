...
#설문
def survey1():
    global window_thumnails, window_survey1

    window_thumnails.destroy()
   # window_video1 = Tk()
...
#그냥 이미지나 gif로 대체하는 것에 대한 의견을 여쭤보고 싶습니다
    ...
    next1 = tk.Button(window_survey1, text="다음",command=gif)
    next1.pack()
...
def gif():
    ...
    global window_survey1, cat

    window_survey1.destroy()
    cat = Tk()
    ...
    def update(ind):
        ...
        cat.after(100, update, ind)
    label = Label(cat)
    label.pack()
    cat.after(0, update, 0)
    cat.mainloop() 
